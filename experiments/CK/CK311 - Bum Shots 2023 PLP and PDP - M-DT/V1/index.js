        ...
        window.optimizely.get('utils').observeSelector(CK311.targetProducts, function(product) {
            const productId = product.getAttribute('data-primarycolour');
             if (product.classList.contains('product-tile-static')) {
                product.querySelector('.ProductImage').style.backgroundImage = `url(https://calvinklein-eu.scene7.com/is/image/CalvinKleinEU/${productId}_alternate1?$b2c_plp_listing_2000$)`;
                ...
                window.optimizely.get('utils').waitUntil(function() {
                    ...
                    product.querySelectorAll('.slick-slide')[1].querySelector('.ProductImage').style.backgroundImage = `url(https://calvinklein-eu.scene7.com/is/image/CalvinKleinEU/${productId}_alternate1?$b2c_plp_listing_2000$)`;
                    product.querySelectorAll('.slick-slide')[2].querySelector('.ProductImage').style.backgroundImage = `url(https://calvinklein-eu.scene7.com/is/image/CalvinKleinEU/${productId}_main?$b2c_plp_listing_2000$)`;
                    product.querySelectorAll('.slick-slide')[5].querySelector('.ProductImage').style.backgroundImage = `url(https://calvinklein-eu.scene7.com/is/image/CalvinKleinEU/${productId}_alternate1?$b2c_plp_listing_2000$)`;

                    product.querySelectorAll('.slick-slide')[1].querySelector('.ProductImage').classList.add('CK311_changed_product_image');
                    product.querySelectorAll('.slick-slide')[2].querySelector('.ProductImage').classList.add('CK311_changed_product_image');
                    product.querySelectorAll('.slick-slide')[5].querySelector('.ProductImage').classList.add('CK311_changed_product_image');
                    product.classList.add('CK311_changed_product');
                });
            }
        });

        window.optimizely.get('utils').observeSelector('[data-testid="ProductImages-component"] [class*=Carousel_slide__][data-slide="true"]:nth-child(2)', function(image) {
            image.classList.add('exp-pdp');
       });
    }
};
CK311.init();