const Experiment = (() => {
  ...
  const getCopy = () => {
      ...
      const translations = {
        ...
        nl: {
            ...
            textCake: `Verjaardag verrassingen`,
        ...
        de: {
          ...
          textCake: `Geburtstag Überraschungen`,
  ...
  const addStickyBtn = () => {
      ...
      //Add content to the btn
      newsBTN.innerHTML = `<span>${textBtn}</span><img id="closeNewsBtn" src="${icons.xButton}" />`;
              
      //Place it       
        document.body.appendChild(newsBTN);
             
      //Event listener to the button
    function scrollHideBtn(){
        const banner = document.querySelector("[data-testid='NewsletterFooter']");
       //const titleBanner = document.querySelector('h3[class*="newsletter-footer__title_"]');
    ...
    function closeBtn(){
      ...
      newsBTN.removeEventListener('click', closeBtn);
      document.removeEventListener('scroll', scrollHideBtn);
    }
     waitForElement('#closeNewsBtn', () => {
        document.querySelector('#closeNewsBtn').addEventListener('click', closeBtn);
     });
    
     //Event listener: when banner appears, the btn display is none
    waitForElement('.newsletterBtn', () => {
      document.addEventListener('scroll', scrollHideBtn);
    });

      //For smooth scroll event
      waitForElement(".newsletterBtn span", () => {
        ...
        //   }
          const link = document.querySelector(".newsletterBtn span");
          link.addEventListener("click", () => {
              let offsetTop = document.querySelector('#newsletterFooterCart').offsetTop - document.querySelector('header').offsetHeight;
...
utils.waitForElement('[data-testid="shoppingBagUspBanner"]').then(function () {
Experiment.start();
});