const TH177 = {
    ...
    init: function() {
        let products = TH177.getTargetProducts();

        for (let i = 0; i < products.length; i++) {
            products[i].sort(TH177.compare);
            products[i] = TH177.filterProductList(products[i]);
    ...
    getTargetProducts: function() {
        let productListContainers = [];
        for (let i = 0; i < document.querySelectorAll('[data-testid="product-list-items"]').length; i++) {
            let productList = [];
            for (let j = 0; j < TH177.productSKUs.length; j++) {
                if (document.querySelectorAll('[data-testid="product-list-items"]')[i].querySelector(`[data-uid=${TH177.productSKUs[j].sku}]`)) {
                    let currentProductListIndex = parseInt(document.querySelectorAll('[data-testid="product-list-items"]')[i].querySelector(`[data-uid=${TH177.productSKUs[j].sku}]`).getAttribute('data-index'));
                    productList.push({key: j, productListIndex: currentProductListIndex});
                }
            }
            productListContainers.push(productList);
        }
        return productListContainers;
    },
    compare: function(a, b) {
        if ( a.productListIndex < b.productListIndex ){
            return -1;
        }
        if ( a.productListIndex > b.productListIndex ){
            return 1;
    ...
    calculateSpaceBetweenProducts: function(previousProductIndex) {
        const spacingValues = {
            twoColLayout: [6, 5],
            threeColLayout: [9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7, 9, 8, 7],
            fourColLayout: [12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9, 12, 11, 10, 9]
        };
    
        let minSpaceBetweenProducts = 0;
    
        if (document.querySelectorAll('[data-testid="product-list-items"] [data-two]').length || document.documentElement.clientWidth <= 740) {
            minSpaceBetweenProducts = previousProductIndex % 2 === 0 ? spacingValues.twoColLayout[0] : spacingValues.twoColLayout[1];
        } else if (document.querySelectorAll('[data-testid="product-list-items"] [data-three]').length) {
            minSpaceBetweenProducts = spacingValues.threeColLayout[previousProductIndex];
        } else {
            minSpaceBetweenProducts = spacingValues.fourColLayout[previousProductIndex];
        }
    
        return minSpaceBetweenProducts; 
    },
    filterProductList: function(productList) {
        let filteredProducts = [productList[0]];
        let i = 0;
        let j = 1;
    
        productList.forEach(function(product) {
            if (j < productList.length) {
                let minSpaceBetweenProducts = TH177.calculateSpaceBetweenProducts(filteredProducts[i].productListIndex);
                if (productList[j].productListIndex - filteredProducts[i].productListIndex >= minSpaceBetweenProducts && 
                    filteredProducts.length < TH177.state.maxNumOfVideos) {
                    filteredProducts.push(productList[j]);
                    i++;
                }
    ...
    buildVideoMarkup: function(products) {
        for (let i = 0; i < products.length; i++) {
            let productVideo = document.createElement('div');
            productVideo.setAttribute('class', 'TH177_product_video_container');
        
            productVideo.innerHTML = `
                <video class="TH177_product_video" loop="" width="100%" poster="${TH177.productSKUs[products[i].key].poster}" preload="metadata" muted playsinline loop>
                    <source src="${TH177.productSKUs[products[i].key].video}" type="video/mp4">
            ...
            products[i].video = productVideo;
        }

        return products;
    },
    insertVideo: function(products, productListContainerIndex) {
        if (document.querySelectorAll('[data-testid="product-list-items"]')[productListContainerIndex].querySelectorAll('.TH177_product_video_container').length)  {
            document.querySelectorAll('[data-testid="product-list-items"]')[productListContainerIndex].querySelectorAll('.TH177_product_video_container').forEach(function(productVideo) {
                productVideo.parentElement.removeChild(productVideo);
            });
        }

        products.forEach(function(product) {
            if (!document.querySelector(`[data-uid=${TH177.productSKUs[product.key].sku}] .TH177_product_video_container`)) {
                document.querySelector(`[data-uid=${TH177.productSKUs[product.key].sku}] .product-image__plp`).insertBefore(product.video,  document.querySelector(`[data-uid=${TH177.productSKUs[product.key].sku}] .product-image__plp`).firstElementChild);
    ...
    playMobileVideoOnScroll: function() {
    ...
    waitForProductListElement: function() {
        ...
        TH177.utils.observeSelector('[data-testid="product-list-items"]', function() {
            ...
            if (!document.querySelector('.TH177_product_video_container')) {
                TH177.init();
            }
        });
    }
